...
GSEA_pl = plotEnrichment(pathways[["GO_WHITE_FAT_CELL_DIFFERENTIATION"]], ranks, ticksSize = 0.5) + labs(title="GO_WHITE_FAT_CELL_DIFFERENTIATION") + 
  theme(plot.title = element_text(size = 20, face = "bold",hjust = 0.5), axis.title.x = element_text(size = 17), 
        axis.title.y = element_text(size = 17), axis.text.x = element_text(size = 14), axis.text.y = element_text(size = 14))+ geom_line(size = 1.5, color = "green")
